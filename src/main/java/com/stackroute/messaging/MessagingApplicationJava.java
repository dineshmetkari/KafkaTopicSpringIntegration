 ...
 * Creates the kafka topic of given name and can send and receive messages on the topic.
 *
 * @author Dinesh Metkari (dineshmetkari@gmail.com)
 */
@EnableIntegration
@SpringBootApplication
public class MessagingApplicationJava {
     ...
     * Refer to {@link org.springframework.boot.context.properties.ConfigurationProperties}!
     */
    @Component
    ...
    public static class ProducerConfiguration {
        ...
        CommandLineRunner kickOff(@Qualifier(OUTBOUND_ID + ".input") MessageChannel in) {
            return args -> {
                for (int i = 0; i < 10; i++) {
                    in.send(new GenericMessage<>("JAVA Message No: " + i));
                    log.info("JAVA Message No: " + i);
                }
            };
        }
    ...
    public static void main(String[] args) {
        ...
        SpringApplication.run(MessagingApplicationJava.class, args);
